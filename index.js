...
firstForm.addEventListener("submit", function (e) {
  getSearchResult(e, firstSearchUl);
});
secondForm.addEventListener("submit", function (e) {
  getSearchResult(e, secondSearchUl);
});
...
function getSearchResult(event, targetUl) {
  ...
  if (searchBar.value != "") {
    fetch(
      `https://youtube.googleapis.com/youtube/v3/search?part=snippet&q=${searchBar.value}&key=${key}&maxResults=3`
    )
      ...
      .then(function (data) {
        displaySearchResult(data, targetUl);
      });
  } else {
...
function filterUniqeChannels(searchList) {
  let unique = {};
  let filtered = searchList.filter(
    (item) => !unique[item.snippet.title] && (unique[item.snippet.title] = true)
  );
  return filtered;
}
...
function displaySearchResult(searchList, targetUl) {
  ...
  for (item of searchList) {
    ...
    li.addEventListener("click", function (e) {
      getChannelData(li.data, targetUl);
    });
  }
}
//function(data){
...
function displayCompare() {
  ...
  if (
    channel1.statistics.subscriberCount > channel2.statistics.subscriberCount
  ) {
